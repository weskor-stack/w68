                            ...
                            try:
                                ...
                                while True:
                                    ...
                                    if len(name_piece) > 27:
                                        url_api_units = url_api_unit.replace("serial_number", name_piece)

                                        # INVOKE API UNIT
                                        try:
                                            response_unit = requests.get(url_api_units, timeout=30)
                                        except Exception as e:
                                            safe_insert(f"Error fetching unit data: {e}", "red")
                                            logging.error(f"Error fetching unit data: {e}")
                                            conn.send("FAILED".encode('UTF-8'))
                                            break
                                        
                                        if response_unit.status_code != 200:
                                            safe_insert(f"API request failed with status code: {response_unit.status_code}", "red")
                                            logging.error(f"API request failed with status code: {response_unit.status_code}")
                                            conn.send("FAILED".encode('UTF-8'))
                                            break
                                        ...
                                        try:
                                            ...
                                            safe_insert(f"Error parsing JSON: {e}", "red")
                                            logging.error(f"Error parsing JSON: {e}")
                                            conn.send("FAILED".encode('UTF-8'))
                                            break

                                        if not json_data_unit or 'data' not in json_data_unit or not json_data_unit['data']:
                                            safe_insert(f"No data found for ISN: {name_piece}", "red")
                                            logging.warning(f"No data found for ISN: {name_piece}")
                                            conn.send("FAILED".encode('UTF-8'))
                                            break
                                        ...
                                        if not part_number_unit:
                                            safe_insert(f"Part number not found for ISN: {name_piece}", "red")
                                            logging.warning(f"Part number not found for ISN: {name_piece}")
                                            conn.send("FAILED".encode('UTF-8'))
                                            break

                                        interlocking_json_api = interlocking_json.interlocking_station_30(
                                            name_piece,
                                            part_number_unit
                                        )
                                        ...
                                        try:
                                            ...
                                            safe_insert(f"Error invoking interlocking API: {e}", "red")
                                            logging.error(f"Error invoking interlocking API: {e}")
                                            conn.send("FAILED".encode('UTF-8'))
                                            break

                                        if response_interlocking.status_code != 200:
                                            safe_insert(f"Interlocking API request failed with status code: {response_interlocking.status_code}", "red")
                                            logging.error(f"Interlocking API request failed with status code: {response_interlocking.status_code}")
                                            conn.send("FAILED".encode('UTF-8'))
                                            break
                                        ...
                                        conexion.piece_store2(name_piece,part_number_unit)
                                        ...
                                        pantalla_final = (
                                            f"Command received-> {cadena} part: {name_piece}\nCommand PASSED\n"
                                            f"[API UNIT RESPONSE]:\n{json.dumps(json_data_unit, indent=4, ensure_ascii=False)}\n\n"
                                            f"[INTERLOCKING JSON]:\n{json.dumps(interlocking_json_api, indent=4, ensure_ascii=False)}"
                        ...
                        if len(option) == 6 and option[-1] == '1/':
                            ...
                            if duration == "PASSED":
                                ...
                                try:
                                    ...
                                    component_enabled = conexion.configuradorst30() 
                                    component_enabled = component_enabled[4]

                                    traceability_payload = traceability_json.traceability_station_30(
                                        option[4],
                                        ""
                                    )
